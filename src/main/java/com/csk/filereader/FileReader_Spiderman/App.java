	...
	public static void main( String[] args )
    {
    	...
    	try(Scanner scan=new Scanner(new FileReader("Spider-Man.Far.from.Home.srt") )){
    		...
    		//		System.out.println("sec *"+hrs[2]);
    				...
    				double x=Integer.parseInt(hrs[2].substring(0,2))+(Integer.parseInt(hrs[2].substring(3,6)))*(0.001)+a; 				
    		//		System.out.println("x =" + x);
    				//System.out.println("x = "+ new DecimalFormat("0.000").format(x-60));
    				if(x>=60){
    					
    					hrs[2]=(Double.toString((x-60))).substring(0,4);
    					hrs[1]=Integer.toString(Integer.parseInt(hrs[1])+1);
    				}else{
    					
    					hrs[2]=Double.toString(x);
    				}
    		...
    		//		System.out.println("Eski Saat Aralığı  " +lineinfo);
    				System.out.println("Yeni Düzenlenmiş saat : "+hrs[0]+":"+hrs[1]+":"+hrs[2].substring(0,4)+" --> "+hrs_end[0]+":"+hrs_end[1]+":"+hrs_end[2].substring(0,4) );
